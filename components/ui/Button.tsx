...
/**
 * Bevora button. Variants: primary (ink), accent (gold), secondary (outline),
...
export function Button({
  ...
  const sizes: Record<Size, React.CSSProperties & { radius: string }> = {
  ...
  return (
    <button
      ...
      style={{
        ...
        transition:
          "background var(--dur-fast) var(--ease-out), box-shadow var(--dur-base) var(--ease-out), transform var(--dur-fast) var(--ease-out), border-color var(--dur-fast) var(--ease-out)",
        outline: "none",
        ...style,
      }}
      {...rest}
